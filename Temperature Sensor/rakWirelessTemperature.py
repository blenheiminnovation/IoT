...
def findTemp(tempPin):
    ADCValue = tempPin.read_u16() #Reading the value in 16bits unsigned
    V0 = (ADCValue / 65535) * 3.3 #Calculating the reference voltage, using the ratio between the measured value and the maximum ADC value. The multiplying the input voltage by this ratio
    
    resistance = ((3.3/V0) - 1) * (1 / 44900) #Using the input and reference voltage, as well as the series resistance
    resistance = 1 / resistance 

    steinhart = resistance / 10000 #Using the calculated, series and nominal values in the Steinhart equation to find the temperature in Kelvin
    ...
    while(decoded_data != "OK Join Success\r\n"):
        try:
            ...
            if data:
                ...
                while(1):
                    led.value(1)
                    temperature = round(findTemp(tempPin), 2)
                    temperatureString = ("TMP~~TST~~" + temperature + "~~0~~0") #This acts as the payload for trasnmission and is in the structure: Sensor Type, Sensor Name, Value 1, Value 2, Value 3
