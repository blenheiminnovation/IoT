...
def ultra():
   ...
   while echo.value() == 0: 
       signaloff = utime.ticks_us() #If no echo pulse isn't recieved, signaloff is updated to contain a timestamp in microseconds
       
   while echo.value() == 1:
       signalon = utime.ticks_us() #If an echo pulse is recieved, signalon is updated to store the current timestamp in microseconds
       
   timepassed = signalon - signaloff #Stores the total time taken for the pulse to echo back off of the object
   distance = (timepassed * 0.0343) / 2 #Converts the time passed into distance, using the speed of sound (343.2ms/1 or 0.0343cm/microsecond)
   return distance
...
while(1):
    distance = round(ultra(), 2) #Constantly checks the distance of the nearest object to the sensor
    if distance <= 15.00: #If the object becomes closer than 15cm, the distance value is transmitted
        led.value(1)
        distanceString = "US~~Testing~~" + str(distance) + "~~" + " " + "~~" + " " #Only one value is outputted by the sensor so the Value 2 and Value 3 fields are left blank
